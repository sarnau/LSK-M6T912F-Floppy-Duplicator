...
**All resolved.** The last hardware unknown — port `0xC0` — was pinned down by **continuity probing of
the board**; the board photo + datasheets had resolved the rest.

- **Port `0xC0` = the high byte of the DRAM bank address** `[V — probed]` — *not* a drive-control latch.
  Continuity confirms it loads latch **U66** (data-fed via the U58 buffer), whose outputs feed the DRAM
  row/high address through the `74HCT157` mux, and whose write strobe comes from decoder **U60**
  (`74HCT138`, decoding A4–A6 enabled by A7): **Y3 → `0xB0`** (U65), **Y4 → `0xC0`** (U66). So the DRAM
  bank is a **`{0xC0, 0xB0}` pair** — `0xB0` the dynamic low byte, `0xC0` the high byte — which is exactly
  why boot writes `0xFF` to *both*, adjacently (`0x0010`/`0x0012`), to point the window at the
  program-RAM mirror. (Supersedes the earlier guess that grouped `0xC0` with the `0xC2/0xC3/0xC6`
  drive-control ports; those use a different decoder.) See §5 and the README *DRAM interface* section.
